...
func (h *Handler) GetProfileTournaments(c *gin.Context) {
	// ctx, cancel := context.WithCancel(c.Request.Context())
	// 	defer cancel()

	// 	lastID := c.Query("last_id")
	// 	userID := c.Param("userID")
	// 	sport, competitorType, limit, err := getProfileTournamentsValidateQueries(c)
	// 	if err != nil {
	// 		customerrors.ErrorResponse(err, c)
	// 		return
	// 	}

	// 	tournaments, err := h.profile.GetProfileTournaments(ctx, userID, *sport, *competitorType, limit, lastID)
	// 	if err != nil {
	// 		customerrors.ErrorResponse(err, c)
	// 		return
	// 	}

	// 	c.JSON(http.StatusCreated, gin.H{"data": tournaments, "status": http.StatusOK, "message": "Competitor finded!"})
	// }

	// func getProfileTournamentsValidateQueries(c *gin.Context) (*model_utils.SPORT, *model_utils.COMPETITOR_TYPE, int, error) {
	...
	// 	sportParsed, competcompetitorTypeParsed, err := getProfileTournamentsQueriesParser(sport, competitorType)
	// 	if err != nil {
	// 		return nil, nil, 0, err
	// 	}
	...
	// func getProfileTournamentsQueriesParser(sport, competitorType string) (model_utils.SPORT, model_utils.COMPETITOR_TYPE, error) {
