...
func (h *Handler) GetUserTournaments(c *gin.Context) {
	ctx, cancel := context.WithCancel(c.Request.Context())
	defer cancel()

	lastID := c.Query("last_id")
	userID := c.Param("userID")
	sport, limit, err := getUserTournamentsValidateQueries(c)
	if err != nil {
		customerrors.ErrorResponse(err, c)
		return
	}

	categories, err := h.tournament.GetUserTournaments(ctx, userID, sport, limit, lastID)
	if err != nil {
		customerrors.ErrorResponse(err, c)
		return
	}
...
func getUserTournamentsValidateQueries(c *gin.Context) (model_utils.SPORT, int, error) {
	...
	sportParsed, err := getUserTournamentsQueriesParser(sport)
	if err != nil {
		return "", 0, err
	}
...
func getUserTournamentsQueriesParser(sport string) (model_utils.SPORT, error) {
