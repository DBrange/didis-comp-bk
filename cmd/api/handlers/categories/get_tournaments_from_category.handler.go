...
func (h *Handler) GetTournamentsFromCategory(c *gin.Context) {
	...
	competitors, err := h.category.GetTournamentsFromCategory(ctx, categoryID, *sport, *competitorType, limit, lastID)
	if err != nil {
		customerrors.ErrorResponse(err, c)
		return
	}
...
func getTournamentsFromCategoryValidateQueries(c *gin.Context) (*model_utils.SPORT, *model_utils.COMPETITOR_TYPE, int, error) {
...
func getTournamentsFromCategoryQueriesParser(sport, competitorType string) (model_utils.SPORT, model_utils.COMPETITOR_TYPE, error) {
